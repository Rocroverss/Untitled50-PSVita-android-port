...
/* ----------- NEW: quick detector for HLSL/Cg-like input ---------- */
static GLboolean glsl_looks_hlsl_like(const char *src) {
  if (!src) return GL_FALSE;
  if (strstr(src, ": POSITION") || strstr(src, ":POSITION")) return GL_TRUE;
  if (strstr(src, ": COLOR")    || strstr(src, ":COLOR"))    return GL_TRUE;
  if (strstr(src, ": TEXCOORD") || strstr(src, ":TEXCOORD")) return GL_TRUE;
  if (strstr(src, ": PSIZE")    || strstr(src, ":PSIZE"))    return GL_TRUE;
  if (strstr(src, "tex2D("))                                 return GL_TRUE;
  if (strstr(src, "struct _Output"))                         return GL_TRUE;
  if (strstr(src, ") : COLOR") || strstr(src, "): COLOR") ||
      strstr(src, "):COLOR")   || strstr(src, ") :COLOR"))   return GL_TRUE;
  return GL_FALSE;
}
/* ----------------------------------------------------------------- */
...
void glsl_translator_process(shader *s, GLsizei count, const GLchar *const *string, const GLint *length) {
	...
	/* ----------- NEW: short-circuit for HLSL/Cg input ----------- */
	if (glsl_looks_hlsl_like(out)) {
#ifdef DEBUG_GLSL_TRANSLATOR
		vgl_log("%s:%d %s: Detected HLSL/Cg-like shader. Skipping GLSL->Cg header and rewrites.\n",
		        __FILE__, __LINE__, __func__);
#endif
		s->source = (char *)vglMalloc(strlen(out) + 1);
		vgl_fast_memcpy(s->source, out, strlen(out) + 1);
		vglFree(out);
		...
		while (str2) {
			...
			*end = 0;
			if (glsl_sema_mode == VGL_MODE_GLOBAL) {
				...
				if (idx2 != -1)
					vglAddSemanticBinding(start, idx2, VGL_TYPE_TEXCOORD);
			} else {
				glsl_reserve_texcoord_bind(idx2, start);
			}
			*end = ',';
#ifndef SKIP_ERROR_HANDLING
			if (idx2 == -1) {
				idx2 = MAX_CG_TEXCOORD_ID - 1;
				vgl_log("%s:%d %s: An error occurred during GLSL translation (TEXCOORD overflow).\n", __FILE__, __LINE__, __func__);
			}
#endif
			*str2 = '0' + idx2;
		...
		while (str2) {
			...
			*end = 0;
			if (glsl_sema_mode == VGL_MODE_GLOBAL) {
				...
				if (idx2 != -1)
					vglAddSemanticBinding(start, idx2, VGL_TYPE_COLOR);
			} else {
				glsl_reserve_color_bind(idx2, start);
			}
			*end = ',';
#ifndef SKIP_ERROR_HANDLING
			if (idx2 == -1) {
				idx2 = MAX_CG_COLOR_ID - 1;
				vgl_log("%s:%d %s: An error occurred during GLSL translation (COLOR overflow).\n", __FILE__, __LINE__, __func__);
			}
#endif
			*str2 = '0' + idx2;
